...
type Props = { filters: NotaDebitoFilters; onChange: (filters: NotaDebitoFilters) => void; onReset: () => void }

export function NotasDebitoFilters({ filters, onChange, onReset }: Props) {
  const { data: motivos = [], isLoading } = useMotivosNotaDebito()
  const update = (patch: Partial<NotaDebitoFilters>) => onChange({ ...filters, ...patch, page: 1 })
      ...
      <CardContent className="p-4">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Select value={filters.estado || "all"} onValueChange={(value) => update({ estado: setSelectValue(value) })}>
            <SelectTrigger><SelectValue placeholder="Estado ND" /></SelectTrigger>
          ...
          <div className="relative"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input className="pl-9" placeholder="Numero ND" value={filters.numero ?? ""} onChange={(event) => update({ numero: event.target.value })} /></div>
