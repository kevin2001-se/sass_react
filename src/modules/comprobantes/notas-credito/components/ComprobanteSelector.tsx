...
export function ComprobanteSelector({ value, error, onChange, disabled }: Props) {
  const [query, setQuery] = useState("")
  const { data: optionsData = [], isLoading } = useQuery({
    queryKey: ["notas-credito", "comprobantes-aceptados", query],
    queryFn: () => notaCreditoService.searchComprobantesAceptados(query),
  })

  const selectedQuery = useQuery({
    queryKey: ["sunat-comprobante", value],
    queryFn: () => notaCreditoService.getComprobante(value as number),
